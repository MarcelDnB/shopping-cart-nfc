...
public class DatabaseVerticle extends AbstractVerticle {
	...
	Long idUsuarioCreado; /*
							 * Dentro del ESP32 primero se va a ejecutar la funcion que a�ade un usuario y
							 * dentro de esta funci�n asignamos a esta variable el id de este,
							 * posteriormente el ESP32 ejecuta la segunda funci�n en la que guardamos las
							 * intolerancias introducidas por el usuario junto al id (que vamos a obtener de
							 * esta variable).
							 */
	...
	private void putUsuario(RoutingContext routingContext) {
		...
		mySQLPool.preparedQuery("INSERT INTO usuario (idComercio) VALUES (?)",
				Tuple.of(usuarioIntolerancias.getIdComercio()), handler -> {
	...
	private void putIntoleranciasUsuario(RoutingContext routingContext) {
		...
		for (Integer i : usuarioIntolerancias.getIntolerancias()) {
			mySQLPool.preparedQuery("INSERT INTO intoleranciasusuario (idIntolerancia, idUsuario) VALUES (?,?)",
					Tuple.of(i, idUsuarioCreado), handler -> {
						if (handler.succeeded()) {
							System.out.println(handler.result().rowCount());
	...
	private void putWifiScan(RoutingContext routingContext) {
		...
		mySQLPool.preparedQuery("INSERT INTO redeswifi (SSID, PWR, captureTime, idComercio) VALUES (?,?,?,?)",
				Tuple.of(wifiReading1.getSSID(), wifiReading1.getPower(), wifiReading1.getTimestamp(),
						wifiReading1.getIdComercio()),
				handler -> {
					if (handler.succeeded()) {
						System.out.println(handler.result().rowCount());
	...
	private void getIntolerances(RoutingContext routingContext) {
		mySQLPool.query(
				"select idIntolerancia from intoleranciasingrediente natural join ingrediente natural join ingredientesproducto"
